...
class threadsafeStack {
   ...
   public:
    ...
    void pop(T &t) {
        // std::lock_guard: 异常安全
        std::lock_guard<std::mutex> lock(m_mutex);
        if (m_stack.empty()) throw emptyStackException();
        t = m_stack.top();
        m_stack.pop();
    }
    ...
    std::shared_ptr<T> pop() {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (m_stack.empty()) throw emptyStackRunError();
    ...
    template <typename U>
    friend void swapWithDeadLock(threadsafeStack<U> &left,
                                 threadsafeStack<U> &right);
    template <typename U>
    friend void swapWithoutDeakLock(threadsafeStack<U> &left,
                                    threadsafeStack<U> &right);
    template <typename U>
    friend void swapWithScopeLock(threadsafeStack<U> &left,
                                  threadsafeStack<U> &right);
    template <typename U>
    friend void swapWithUniqueLock(threadsafeStack<U> &left,
                                   threadsafeStack<U> &right);
};
