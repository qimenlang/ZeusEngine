...
template <class U>
void swapWithoutDeakLock(threadsafeStack<U> &left, threadsafeStack<U> &right) {
    if (&left == &right) return;
    // 同时锁住两个互斥，防止死锁
...
void increment() {
    for (int i = 0; i < 100000; ++i) ++shared_value;  // 数据竞争！
}
...
void safe_increament() {
    ...
    for (int i = 0; i < 100000; ++i) shared_value2++;
}
...
void mutexDataRaceTest() {
    ...
    t1.join();
    t2.join();
    std::cout << "mutexDataRaceTest shared_value2:" << shared_value2
              << std::endl;
}
